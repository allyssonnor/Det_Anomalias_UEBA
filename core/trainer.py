...
class Trainer:
    ...
    def _compute_f2_score(self, y_true, y_pred):
        if y_true is None or len(y_true) == 0: return 0.0
    ...
    def _prepare_and_scale_X(self, group, user_id, fit=False, feature_columns=None):
        ...
        if self.feature_columns is not None:
            ...
            if fit: self.feature_columns = X_num.columns.tolist()

        X_arr = X_num.values.astype(np.float32)
        if X_arr.shape[0] == 0: return X_arr
    ...
    def _build_sequences(self, df, fit=False, return_labels=False, feature_columns=None):
        ...
        for user_id, group in df.groupby("UserID"):
            ...
            if len(X) < self.sequence_length: continue
        ...
        if not X_all: return np.array([]), np.array([]), np.array([]), None
        
        return np.concatenate(X_all), np.concatenate(indices_all), np.concatenate(users_all), np.concatenate(labels_all) if labels_all else None
    ...
    def _calculate_scores(self, X):
        ...
        if is_if_model or "isolationforest" in model_type_name or "iforest" in model_type_name:
            if hasattr(self.model, "decision_function"): return -self.model.decision_function(X)
            elif hasattr(self.model, "score_samples"): return -self.model.score_samples(X)
            
        if hasattr(self.model, "score"):
            scores = self.model.score(X)
            if self.config.get("model", {}).get("auto_invert_scores", False) and np.mean(scores) < 0: return -scores
            return scores
    ...
    def run(self, train_df, val_df, test_df, feature_columns=None):
        ...
        if do_resume and os.path.exists(model_path) and os.path.exists(meta_path):
            ...
            if hasattr(self.model, "load"): self.model.load(model_path)
        else:
            ...
            X_train, _, _, y_train = self._build_sequences(train_df_pure, fit=True, return_labels=True, feature_columns=feature_columns)
            if y_train is None: y_train = np.zeros(len(X_train))
            ...
            if is_supervised and target_attack and "AnomalyType" in val_df.columns:
                ...
                X_val_inj, _, _, y_val_inj = self._build_sequences(val_filtered, fit=False, return_labels=True)
        ...
        if target_attack and "AnomalyType" in val_df.columns:
            print(f"   🔍 [ESPECIALISTA] Filtrando validação apenas para tráfego normal e '{target_attack}'...")
            mask = (val_df[target] == 0) | (val_df["AnomalyType"] == target_attack)
            val_df = val_df[mask].copy()

        X_val, _, val_users, y_val = self._build_sequences(val_df, fit=False, return_labels=True)
        if X_val is None or len(X_val) == 0: raise ValueError("❌ Erro Crítico: Conjunto de VALIDAÇÃO vazio.")

        val_scores = self._calculate_scores(X_val)

        if y_val is not None and np.sum(y_val) > 0: print(f"   ✅ Validação contém {int(np.sum(y_val))} janelas anômalas.")
        else: print(f"   ❌ ATENÇÃO: Validação livre de anomalias registradas!")
        ...
        if fixed_threshold is not None:
            ...
            if thresh_mode == "per_user":
                for uid in np.unique(val_users):
                    ...
                    if np.sum(user_mask) >= 5: self.user_thresholds[str(uid)] = float(np.percentile(val_scores[user_mask], best_p))
        else:
            if y_val is not None and np.sum(y_val) > 0:
                ...
                for p in np.arange(50.0, 99.6, 0.5):
                    ...
                    if thresh_mode == "per_user":
                        u_t_dict = {str(uid): np.percentile(val_scores[val_users==uid], p) for uid in np.unique(val_users) if np.sum(val_users==uid) >= 5}
                    ...
                    if f2 > best_f2: best_f2, best_p = f2, p
        ...
        return {"val_f2": float(best_f2) if best_f2 != -1 else -1.0, "global_threshold": self.global_threshold}
