#!python scripts/extract_anomalies.py --output_dir "results/LAST_RUN_FINAL/full/marathon_seed42/Synthetic_MLP_Single_CPU_threshold_modeper_user_seq_len20_max_iter50/eval_mixed" --test_csv "data/output_thesis_marathon/synthetic/test/test.jsonl"
import argparse
import json
import yaml
import glob
...
def main():
    parser = argparse.ArgumentParser(description="Auditoria estrita alinhada ao classification_report com leitura inteligente de YAML.")
    ...
    # 1. Se o gabarito estiver separado, fazemos a mesclagem aqui
    ...
    if target_col not in test_df.columns:
        ...
        print("Passe o argumento --labels_csv se o JSONL não tiver a coluna original.")
        return

    # 2. INTELIGÊNCIA DE BUSCA DA JANELA TEMPORAL (ESTADO DA ARTE)
    seq_len = args.seq_len
    
    if seq_len is None:
        # Procura qualquer arquivo .yaml na pasta de saída
        yaml_files = glob.glob(os.path.join(args.output_dir, "*.yaml"))
        
        # Se não achar, procura um nível acima (pasta pai), útil para subpastas como /eval_mixed
        if not yaml_files:
            parent_dir = os.path.dirname(os.path.normpath(args.output_dir))
            yaml_files = glob.glob(os.path.join(parent_dir, "*.yaml"))
            
        if yaml_files:
            config_encontrado = yaml_files[0] # Pega o primeiro YAML que achar
        ...
        # Fallbacks caso o YAML não exista ou não tenha a chave
        if seq_len is None:
            if len(indices) > 0:
        ...
        print(f"⚙️ Janela temporal forçada manualmente pelo usuário: {seq_len}")

    # 3. REPRODUÇÃO EXATA DA LÓGICA DO PIPELINE PRINCIPAL
    y_true_list = []
    ...
    if len(y_true) != len(scores):
        print(f"❌ ERRO DE ALINHAMENTO: Foram gerados {len(y_true)} rótulos, mas existem {len(scores)} scores.")
        return

    # 4. APLICAÇÃO DOS LIMITES
    ...
    print("\n" + "="*60)
    print("📊 RESUMO DOS ACHADOS DA DETECÇÃO")
    print("="*60)
    ...
    print("="*60)
    # Mensagem de salvamento adicionada de volta:
